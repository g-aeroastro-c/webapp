...
export default function ContactPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Optional: Clear the form fields after successful submission (or perceived success)
    setFormData({
      firstName: '',
      lastName: '',
      email: '',
    ...
    // Optional: Provide a simple alert to the user that the message was sent
          ...
          <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div className="bg-white/5 backdrop-blur-sm p-6 sm:p-8 rounded-3xl border border-white/10">
              <form
                onSubmit={handleSubmit}
                className="space-y-6"
                name="club_contact_form"     // ✨ Name for Netlify to identify your form
                method="POST"                // ✨ Specifies the HTTP method for form submission
                data-netlify="true"          // ✨ Tells Netlify to process this form
                netlify-honeypot="bot-field" // ✨ For basic spam protection
              >
                {/* ✨ Hidden honeypot field for spam protection. Place it directly inside the form. */}
                <p style={{ display: 'none' }}>
                  <label>
                    Don’t fill this out if you’re human: <input name="bot-field" />
                  </label>
